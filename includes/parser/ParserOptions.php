...
class ParserOptions {
	...
	public function optionsHash( $forOptions ) {
		...
		if ( is_array( $forOptions ) && in_array( 'math', $forOptions ) )
			$confstr .= $this->mMath;
		else
			$confstr .= '*';
		...
		if ( is_array( $forOptions ) && in_array( 'dateformat', $forOptions ) )
			$confstr .= '!' . $this->getDateFormat();
		
		if ( is_array( $forOptions ) && in_array( 'numberheadings', $forOptions ) )
			$confstr .= '!' . ( $this->mNumberHeadings ? '1' : '' );
		else
			$confstr .= '!*';
		
		if ( is_array( $forOptions) && in_array( 'userlang', $forOptions ) )
			$confstr .= '!' . $this->mUserLang;
		else
			$confstr .= '!*';

		if ( is_array( $forOptions) && in_array( 'thumbsize', $forOptions ) )
		...
		if ( is_array( $forOptions ) && !in_array( 'editsection', $forOptions ) ) {
			$confstr .= '!*';
		} elseif ( !$this->mEditSection ) {
			$confstr .= '!edit=0';
		}
		
		if (  $this->mIsPrintable && is_array( $forOptions ) && in_array( 'printable', $forOptions ) )
			$confstr .= '!printable=1';
